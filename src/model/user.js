...
const populateUser = () => {
  User.deleteMany(() => console.log('Drop User Collection')).then(() => {
    const users =
    [new User({userId: '1', balance: 500}),
      new User({userId: '2', balance: 1000}),
      new User({userId: '3', balance: 700})];
    users.forEach( (user) => {
      user.save().
          then((user) =>
            console.log(`${user.userId} is create successfully with balance equal ${user.balance}`))
          .catch((error) =>
