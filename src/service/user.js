...
const stockExhange = async (userId, stock_id, total, upper_bound, lower_bound, type) =>{
  try {
    ...
    const totalCurrentStockPrice =
    currentStock.price * currentStock.avalibility;
    const time = currentStock.timestamp;
    if (total > currentStock.avalibility) {
      throw new Error('the request more than the available stock');
    }
    if (currentUser.balance < totalCurrentStockPrice) {
      throw new Error('insufficient user ballance');
    }
    if (upper_bound < currentStock.price) {
      throw new Error('the current price exceeds the upper bound');
    }
    if (lower_bound > currentStock.price) {
      throw new Error('the current stock price less than lower bound');
    }
    const transaction =
    new Transaction({user: userId, stockId: stock_id, time, type});
    transaction.save().
        then((transactionn) =>
          console.log(`is create successfully with balance equal`))
        .catch((error) =>
          console.log(`Error is ${error} while initializing users`));
  } catch (err) {
    console.log(err);
    throw err;
  }
